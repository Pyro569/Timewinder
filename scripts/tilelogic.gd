...
func _ready():
	...
	for cell in get_used_cells_by_id(0, 7): # future cam
		...
		get_node("/root/" + get_tree().current_scene.get_name() + "/Camera/Camera2D").position = future_cam
	for cell in get_used_cells_by_id(0, 5): # player start
		...
		get_node("/root/" + get_tree().current_scene.get_name() + "/Player/CharacterBody2D").position = cell * cell_size + (cell_size - player_size) / 2
...
func _process(delta):
	if Input.is_action_pressed("timeTravel") and unixTime >= 2:
		if !time_travelling:
			...
			if future:
				future = false
				get_node("/root/" + get_tree().current_scene.get_name() + "/Camera/Camera2D").position = past_cam
				get_node("/root/" + get_tree().current_scene.get_name() + "/Player/CharacterBody2D").position += Vector2(past_cam - future_cam)
				unixTime = 0
			else:
				future = true
				get_node("/root/" + get_tree().current_scene.get_name() + "/Camera/Camera2D").position = future_cam
